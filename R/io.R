...
#'   adds an `outgroupaf` column to the SNP metadata for use by downstream
#'   outgroup-based filters; otherwise this column is set to `0.5`.
...
#'   supplied, overrides `maxmiss` and `auto_only` (only the supplied SNP
#'   set is kept, ignoring those filters). Default `NULL`.
...
#' @examples
#' \dontrun{
#' afdat = plink_to_afs(prefix, pops = pops)
#' afdat_filtered = discard_from_aftable(afdat, maxmiss = 0, auto_only = TRUE)
#' # afdat_filtered$afs has fewer rows than afdat$afs
#' }
...
read_f2 = function(f2_dir, pops = NULL, pops2 = NULL, type = 'f2',
  ...
  if(verbose) message(paste0('Reading ', type, ' data for ', nrow(popcomb), ' pairs...'))
...
format_info = function(pref, format = NULL) {
  ...
  if(is_ancestrymap_prefix(pref) && is_packed(paste0(pref, '.geno')) || isTRUE(format == 'packedancestrymap')) {
    ...
    # The other fields are NULL for safety: callers that try to use
    # cpp_read_geno / read_table2 directly on a PFILE prefix will fail
    ...
    l$indend = '.psam'
    l$genoend = '.pgen'
    # No cpp_read_geno / cpp_geno_to_afs / cpp_geno_ploidy for PFILE —
    # callers using those must route through the format-aware helpers
    # (.make_block_reader / pfile_to_afs / .pfile_ploidy_probe).
  } else stop('Genotype files not found!')
  l
}
...
# .make_block_reader independently (see PR notes).
...
#' @param f4mode If `TRUE`: f4 is computed from allele frequencies `a`, `b`, `c`, and `d` as `(a-b)*(c-d)`. if `FALSE`, D-statistics are computed instead, defined as `(a-b)*(c-d) / ((a + b - 2*a*b) * (c + d - 2*c*d))`, which is the same as `(P(BABA) - P(ABBA)) / (P(ABBA) + P(BABA))`.
...
f4blockdat_from_geno = function(pref, popcombs = NULL, left = NULL, right = NULL, auto_only = TRUE,
                                ...
                                cm_file = NULL, return_matrices = FALSE, verbose = TRUE) {
  ...
  if(isTRUE(l$is_pfile)) {
    # PFILE metadata path. .read_pvar / .read_psam return tibbles in the
    # same shape the rest of this function expects (snpfile columns
    # SNP/CHR/cm/POS/A1/A2; indfile renamed from IID/FID to ind/pop).
    ...
    snpfile = .read_pvar(paste0(pref, l$snpend))
    if(!is.null(cm_file)) {
      if(verbose) alert_info(paste0('Reading cm from ', cm_file, '...\n'))
    ...
    snpfile = snpfile[, c('SNP', 'CHR', 'cm', 'POS', 'A1', 'A2')]   # drop is_multi
    psam = .read_psam(paste0(pref, l$indend))
    indfile = tibble::tibble(ind = psam$IID, pop = psam$FID)
  } else {
    # integration build uses readr's read_table (PR #101 rename of read_table2)
    indfile = read_table(paste0(pref, l$indend), col_names = l$indnam, col_types = l$indtype, progress = FALSE)
    snpfile = read_table(paste0(pref, l$snpend), col_names = l$snpnam, col_types = 'ccddcc', progress = FALSE)
  }
  ...
  # handles for PFILE).
  #
  # Fork-safety note: pgenlibr external pointers do not survive across
  # furrr::future_map workers (under plan(multisession), workers run in
  # separate R processes communicating via sockets; external pointers
  # don't serialize). This is currently safe because future_map under
  # plan(sequential) — the default — runs in the parent process where
  # the handle is alive. PFILE inputs under plan(multisession) will fail
  # at the worker's first reader() call; production-parallel PFILE
  # support is a follow-up that needs each worker to lazy-open its own
  # handle. Sequential plan + PFILE works as expected.
  ...
  process_block = function(i) {
    ...
    if(is.null(snpwt)) {
      ...
      # Also avoids 32-bit arma indexing overflow at >~2700 SNPs/block on
      # million-popcomb runs.
      rm = cpp_aftable_to_dstatnum_rowmeans(at, p1, p2, p3, p4, modelvec, block_usesnps, allsnps, poly_only)
      # arma::vec wraps to R as a 1-column matrix; flatten with c() so the
      # downstream do.call(rbind, ...) over per-block results gives a
      # (numblocks x npopcomb) matrix rather than (numblocks*npopcomb x 1).
  ...
  if(return_matrices) {
    # Skip the long-format expand_grid build (which would otherwise pay
    # O(npopcomb * nblocks) memory; for dense f4 sets this dominates the
    # whole pipeline at ~250 GB on 1.5M-popcomb runs). Return the
    # underlying nblocks x npopcomb matrices plus the metadata callers
    # need for downstream regression. Used by qpfstats; other callers
    # still get the long-format frame by default.
    return(list(
      .matrices_only = TRUE,
      numer    = numer,
      cnt      = cnt,
