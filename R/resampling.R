...
matrix_jackknife_est_full = function(numer, cnt) {
  ...
  # rel_bl uses ALL n's, regardless of est validity (matches dplyr's mutate
  # which sees the full group; only later filter(is.finite(loo)) drops rows)
  sum_n_all = colSums(cnt)
  sum_n_all[sum_n_all == 0] = NA_real_
  rel_bl = sweep(cnt, 2, sum_n_all, '/')

  # tot drops invalid (NA est), and drops its weight too
  ...
  #     = (sum(tot2) - sum_loo)/n_finite * n_finite + weighted_loo_mean
